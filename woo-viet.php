...
class WooViet
{
    ...
    public function notice_if_not_woocommerce()
    {
        ...
        $message = __(
            'Woo Viet is not running because WooCommerce is not active. Please activate both plugins.',
            'woo-viet'
        );
    ...
    public function main()
    {
        ...
        if ('yes' == $settings['add_onepay_domestic']['enabled']
            ...
            if (defined('DOING_CRON') and DOING_CRON) {
                 ...
                 * @since 1.5.0
                 */
                add_action('wooviet_onepay_domestic_handle_querydr', [
                 ...
                 */
                add_action('wooviet_handle_onepay_querydr', [
                    $wooviet_onepay_domestic,
                    'handle_onepay_querydr'
                ], 10, 1);
            }
        ...
        if ('yes' == $settings['add_onepay_international']['enabled']
             ...
             */
            if (defined('DOING_CRON') and DOING_CRON) {
                add_action('wooviet_onepay_international_handle_querydr', [
        ...
        if ('yes' == $settings['add_province']['enabled'] && 'no' == $settings['use_goship']['enabled']) {
            ...
            if ('yes' == $settings['add_city']['enabled'] && 'no' == $settings['use_goship']['enabled']) {
    ...
    {
        $settings = [ '<a href="' . admin_url('admin.php?page=woo-viet') . '">' . __('Settings', 'woo-viet') . '</a>' ];
